...
#define followPin 10
#define solenoid_1 A1
#define solenoid_2 A0
#define coin_LED A2
#define coinPin 13
#define tapPin 2

//control stuff0(C D E F)
...
bool tap_pin_state = 1;
bool tap_on = 0; //THIS WILL NEED TO CHANGE
bool tap_active = 0;
long last_tap_ms = 0;
uint8_t tap_step = 0;
long tap_times_ms[4] = {0,0,0,0};
...
//riff stuff
uint8_t riff_number[2] = {0,0};
int riff_step[2] = {0,0};
bool riff_play[2] = {0,0};
uint8_t riff_timing[2][2] = {{0,0},{0,0}};
uint8_t last_note[] = {0,0}; 
bool riff_arpeggio = 0;
...
const uint8_t beep_test_bpm_per_level[] = {27, 80, 120, 140, 160, 180, 200, 220, 240};
...
{    
    ...
    if (tap_on && millis() - last_tap_ms > 3000)
    {
        tap_active = 0;
        tap_step = 0;
    }

    if (tap_on && !tap_active)
    {
        bpm_delay_micros = bpm_delay_micros + 2000;
    }
...
void button_check()
{
    ...
    //check tap
    if (digitalRead(tapPin) != tap_pin_state)
    {
        tap_pin_state = !tap_pin_state;
        if (tap_pin_state == 0 && tap_on)
        {
            tap();
...
void tap()
{
    if (tap_active == 0 && tap_step == 0)
        {
            last_tap_ms = millis();
            tap_step++;

        }
    else if (millis() - last_tap_ms > 200 && millis() - last_tap_ms < 3000)
    {
        tap_times_ms[tap_step - 1] = millis() - last_tap_ms;
        last_tap_ms = millis();
        tap_step++;
        if (tap_step > 4)
        {
        ...
        if (tap_active)
        {
            bpm_delay_micros = (tap_times_ms[0]+tap_times_ms[1]+tap_times_ms[2]+tap_times_ms[0])*1000/48;
        }
    }
}
...
void riff_button_update(uint8_t button, bool state)
{
    switch (button)
    {
        ...
        case 10:
            if (state == 1)
            {
                riff_arpeggio = !riff_arpeggio;
            }
        break;
        case 11:
            if (state == 1)
            {
                MIDI.sendNoteOn(0, 6, riff_channel);
            }
            else
            {
                MIDI.sendNoteOn(0, 7, riff_channel);
            }
        break;
        case 12:
            if (state == 1)
...
{   
    ...
    {
        uint8_t drum_note_byte = 0;
        uint8_t drum_velocity_byte = 0;

        if (mutes[5] == 0)
        {
            bitSet(drum_velocity_byte, 0); //if clock on, send clock signal
        }
        
        if (mutes[4] == 0 && sample_number > 0) //if the sample has a number, set the drum_note_byte and velocity_byte accordingly
        {
            drum_note_byte = (sample_number >> 4)*8; 
            drum_velocity_byte = (sample_number & B00001111)*8; //+ 1; //see if bit 0 of velocity is needed
            bitSet(drum_velocity_byte, 1);

            if ((beat_count)%sample_length == 0)
            {
                bitSet(drum_velocity_byte, 2); //sets if sample is starting/repeating
            }
        }
        ...
        {                                            
            ...
            {
                bitSet(drum_note_byte, 2-i);
                bitSet(drum_velocity_byte, 1); // see if bit 0 of velocity is needed
            }
            if ((mode == 1 && button_status[6-i] == 1)&&button_status[0] == 0)
            {
                bitClear(drum_note_byte, 2-i);
            }
        }
        
        if (drum_note_byte > 0 || drum_velocity_byte > 0)
        {
            play_drum(drum_note_byte, drum_velocity_byte);
        }
    }
...
void play_sequence()
{
    if ((beat_count % sequence_timing[0] == 0 && sequence_pending == 0))
    {
        ...
        for (bool loop = 1; loop == 1;)
        {
            ...
            if (command == '*')
            {
                load_riff(string_to_int(sequence_pointers[sequence_number], sequence_step+1, 2),1);
                sequence_step = sequence_step + 2;
            }

            if (command == 'b')
            {
                load_riff(string_to_int(sequence_pointers[sequence_number], sequence_step+1, 2),0);
                sequence_step = sequence_step + 2;
            }
...
void generate_arpeggio()
{
    uint8_t arp = last_chord[0];
    if (beat_count % (drum_timing*2) == 0)
    {
        if ((beat_count/(drum_timing*2)) % 4 == 1 || (beat_count/drum_timing) % 4 == 3)
        {
            arp = arp + 3;
            if(last_chord[1] == 1)
            {
        ...
        else if ((beat_count/(drum_timing*2)) % 4 == 2)
        {
            arp = arp + 7;
            if(last_chord[1] == 3)
            {
        ...
        play_note(arp, riff_channel);
    }
    if (beat_count % (drum_timing*2) == drum_timing)
    {
        stop_note(last_note, riff_channel);
    }
}
...
void generate_bass()
{
    ...
    switch(bass_mode)
    {
        ...
        case 3:
            ...
            if (beat_count % drum_timing == drum_timing/2)
            {
                stop_note(last_chord[0], bass_channel); 
            }
        ...
        case 5:
            if (beat_count % (drum_timing) == 0)
            {
                if ((beat_count/drum_timing) % 4 == 1 || (beat_count/drum_timing) % 4 == 3)
                {
                    arp = arp + 3;
                    if(last_chord[1] == 1)
                    {
...
void load_riff(uint8_t number,bool bass_or_lead)
{
    riff_number[bass_or_lead] = number;
    riff_timing[bass_or_lead][0] = 48/string_to_int(riff_pointers[riff_number[bass_or_lead]],0,2);
    riff_timing[bass_or_lead][1] = string_to_int(riff_pointers[riff_number[bass_or_lead]],3,2);
    riff_step[bass_or_lead] = 5;
    riff_play[bass_or_lead] = 1;
}

void play_riff(bool bass_or_lead)
{
    if (beat_count % riff_timing[bass_or_lead][0] == 0)
    {
        uint8_t note = 0;
        
        for (bool loop = 1; loop == 1;)
        {            
            riff_step[bass_or_lead]++;
            
            char command = pgm_read_byte(riff_pointers[riff_number[bass_or_lead]]+riff_step[bass_or_lead]);
            
            if (command == '!')
            {
                loop = 0;
                riff_play[bass_or_lead] = 0;
            }

            if (command > 64 && command < 72) // NOTE
            {
                note = string_to_root(riff_pointers[riff_number[bass_or_lead]], riff_step[bass_or_lead]);
            }
            ...
            {
                if((last_note[1] == 0 && mutes[2] == 0) && bass_or_lead)
                {
                    play_note(note, riff_channel);  
                }
                if (mutes[3] == 0 && !bass_or_lead)
                {
                    play_note(note+12, bass_channel);
                }
                loop = 0;
            }
        }
    }
    if ((beat_count % riff_timing[bass_or_lead][0] == riff_timing[bass_or_lead][1] && last_note[1] == 0)&& (pgm_read_byte(riff_pointers[riff_number[bass_or_lead]]+riff_step[bass_or_lead]+1) != '.' && bass_or_lead))
    {
        if (mutes[2] == 0)
        {
            stop_note(last_note[1],riff_channel);
        }
    }
}
